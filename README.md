...
- 이번 앱에서, 처음으로 기획/개발 명세서를 작성하는 것부터 시작하여 UI를 그리고 기능을 얹어 출시까지 하나의 앱이 만들어지는 모든 과정을 다뤘습니다. 그러면서 구체적인 계획을 짜기 위해 각 이터레이션마다 할 일과 그에 대한 공수 산정을 진행했는데, 공수 산정을 정말 잘 맞춘 적도 간혹 있었지만 대부분 산정한 시간보다 오래 걸리는 경우가 많았습니다. 공수를 산정할 때 오류 없이 완벽히 끝내게 될 경우를 생각했던 것 같아 다음에는 조금 더 넉넉하게 계획을 세우려고 합니다.
- 짧은 기간에 많은 뷰를 다루면서 반복되는 컴포넌트를 정리하고, 어떻게 하면 더 효율적으로 모듈화를 할지에 대해 고민했습니다. 해당 프로젝트에서는 반복되는 컴포넌트를 Extension으로 만들어 달라지는 부분들을 parameter로 받는 함수를 통해 생성했습니다. 지금껏 간단한 프로젝트를 구현할 때는 문제가 되지 않았으나, 해당 프로젝트에서는 Parameter로 들어가는 값이 꽤 많아져서 비교적 깔끔해 보이지 않았습니다. 다음 프로젝트에서는 디자인 패턴을 조금 더 공부하여 뷰 객체를 지금보다 효과적으로 관리하고 싶습니다.
